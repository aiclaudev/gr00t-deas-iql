...
def load_robocasa_gym_env(env_name, n_envs=1, **kwargs):
    ...
    for index in range(n_envs):
        ...
        if env_kwargs.get("seed") is not None:
            env_kwargs["seed"] += index
        if env_kwargs.get("video_path") is not None:
            env_kwargs["video_path"] = Path(env_kwargs["video_path"]) / f"env_{index}"
        env_fns.append(partial(create_robocasa_gym_env, env_name=env_name, **env_kwargs))
    if n_envs == 1:
...
def create_robocasa_gym_env(
    ...
    # End at the first successful simulator step, including inside an action chunk.
    terminate_on_success: bool = True,
):
    if execute_horizon is None:
        execute_horizon = action_horizon
    if not 1 <= execute_horizon <= action_horizon:
        raise ValueError("execute_horizon must be between 1 and action_horizon")
    ...
    env = RoboCasaWrapper(env, terminate_on_success=terminate_on_success)
    ...
    if record_video:
        ...
        env = RecordVideo(env, video_base_path, disable_logger=True, episode_trigger=episode_trigger, fps=video_fps)
    ...
    env = MultiStepWrapper(
        ...
        n_action_steps=execute_horizon,
...
class RoboCasaWrapper(gym.Wrapper):
    def __init__(self, env, terminate_on_success: bool = True):
        super().__init__(env)
    ...
    def _convert_observation_space(self):
        ...
        new_observation_space["annotation.human.action.task_description"] = gym.spaces.Text(
            max_length=1000, charset=string.printable
        )
    ...
    def step(self, action):
        ...
        info["success"] = self.is_success()["task"]
        # Propagate success as termination before RecordVideo and MultiStepWrapper
        # see this simulator step, so neither runs the remainder of the chunk.
        terminated = bool(terminated or (self.terminate_on_success and info["success"]))
