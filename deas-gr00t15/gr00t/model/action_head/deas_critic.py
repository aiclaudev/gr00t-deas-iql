...
class DEASCriticConfig(PretrainedConfig):
    ...
    # Missing in old checkpoints: online Q was trained on two transforms.
    online_q_feature_passes: int = field(default=2)
...
class DEASCritic(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        if type(config.online_q_feature_passes) is not int or config.online_q_feature_passes not in (1, 2):
            raise ValueError("online_q_feature_passes must be 1 (consistent) or 2 (legacy)")
    ...
    def process_backbone_output(self, backbone_output: BatchFeature) -> BatchFeature:
        ...
        if getattr(self.config, "online_q_feature_passes", 2) == 1:
            # Each loss receives the original features. Returning a new mapping
            # prevents value_loss from changing the subsequent online-Q input.
            return BatchFeature(data={**backbone_output, "backbone_features": backbone_features})
        # Preserve the training semantics of legacy checkpoints when resuming.
