...
# Each distributed process needs its own dynamic-module copies during first import.
if "RANK" in os.environ and "HF_MODULES_CACHE" in os.environ:
    os.environ["HF_MODULES_CACHE"] = str(
        Path(os.environ["HF_MODULES_CACHE"]) / f"rank-{int(os.environ['RANK'])}"
    )
...
class ArgsConfig:
    ...
    seed: int = 42
    """Random seed for initialization, data sampling, and augmentation."""
    ...
    bc_loader: Literal["random", "shard"] = "random"
    """Optional N1.7-style RAM shard cache for single-GPU BC."""
...
def main(config: ArgsConfig):
    ...
    if config.report_to == "wandb":
        ...
        wandb_dir = Path(config.output_dir).expanduser().resolve()
    ...
    if config.bc_loader == "shard":
        if config.num_gpus != 1:
            raise ValueError("BC shard loader requires num_gpus=1")
        from gr00t.data.bc_shard_dataset import BCShardDataset
        train_dataset = BCShardDataset(
...
if __name__ == "__main__":
    ...
    if config.num_gpus == 1:
        # Use the first selected device before CUDA discovery, avoiding DataParallel.
        visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES", "0")
        os.environ["CUDA_VISIBLE_DEVICES"] = visible_devices.split(",")[0].strip()
    ...
    if config.num_gpus == 1:
        # Preserve the caller's GPU selection; the visible device is logical cuda:0.
