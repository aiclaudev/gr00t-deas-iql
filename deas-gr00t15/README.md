...
## B200 cluster evaluation

For the local `bench/` packages, tested environment, smoke checks, and Slurm actor/critic evaluation commands, see [RoboCasa on B200](docs/robocasa_b200.md).

For the prepared actor checkpoint → RoboCasa actions → recorded results pipeline, see
[GR00T actor evaluation pipeline](docs/robocasa_pipeline.md). The planner defaults to dry-run;
preparing this code does not submit evaluations.

## Prepared joint SVF training

For frozen DEAS Q → joint soft-value/policy learning, see
[SVF setup and validation status](docs/SVF_DEAS_PLAN.md). The prepared seed-42
configuration uses 4 GPUs, global batch 128 and 10,000 steps. The launcher defaults
to dry-run; preparing these files does not queue a job after critic completion.

For the prepared **1 GPU × batch 128 vs 4 GPUs × batch 32** worker benchmark,
including Decord/TorchCodec comparisons, see [throughput benchmark](docs/deas_throughput_benchmark.md).
Its launcher defaults to dry-run; speed benchmarks must run through Slurm.
